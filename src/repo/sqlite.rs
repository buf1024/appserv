...
#[async_trait]
impl UserServRepo for SqliteRepo {
    async fn create_user(&self, signup: SignUpReq) -> Result<User> {
        ...
        sqlx::query(
            "insert into user(`user_name`, `email`, `passwd`, `status`, `update_time`) values (?, ?, ?, ?, CURRENT_TIMESTAMP)",
        )
        .bind(&user.user_name)
        .bind(user.email)
        .bind(user.passwd)
    ...
    async fn activate_user(&self, user_id: i64) -> Result {
        ...
        sqlx::query(
            "update user set status = ?, active_time = CURRENT_TIMESTAMP, update_time = CURRENT_TIMESTAMP where id = ?",
        )
